package app.fyreplace.api.sentry.filters;
...
public final class SentryFilter implements ContainerRequestFilter, DynamicFeature {
    ...
    public void filter(final ContainerRequestContext context) {
        ...
        sentryRequest.setQueryString(uriInfo.getQueryParameters().entrySet().stream()
                .map(entry -> entry.getKey() + '=' + String.join(",", entry.getValue()))
                .collect(Collectors.joining("&")));
        sentryRequest.setHeaders(context.getHeaders().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, entry -> String.join(";", entry.getValue()))));
        sentryRequest.setCookies(context.getCookies().entrySet().stream()
